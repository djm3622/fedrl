...
def train_fedavg(env: HeteroBandit, cfg: Config, summary: Dict):
    ...
    fedavg_server = FedAvgServer(global_model=global_model, cfg=cfg)

    client_optimizers = [AdamW(m.parameters(), lr=cfg.lr) for m in client_models]

    for round in range(cfg.num_communication_rounds):

        fedavg_server.broadcast(client_models)

        round_losses = [] # To store losses for this round
        ...
        fedavg_server.aggregate(client_models)
        ...
        summary['fedavg']['losses'].append(float(np.mean(round_losses)))
